...
not interchangeable. The latest public release is currently `v1.3.1`, which
is also the latest release with real-EKS redaction verification (see below), the
...
| `verifiedEKSReleaseVersion` | The most recent release with real-EKS validation or recertification evidence. | Verification copy for current release claims. | Bumped manually, only when a new real-EKS validation or recertification ships. Never tied to the env var — a new release does not imply new validation. |
...
2. "This is the release we most recently validated or recertified against real EKS-derived evidence" (verified-EKS release)
...
Those claims drift apart the moment a release ships without fresh EKS
validation or recertification, or without a fresh feature-demo recording — the site would
...
- Only update `verifiedEKSReleaseVersion` in `site.ts` after re-running the
  real-EKS validation or recertification against the new release.
...
The current release also includes real-EKS-verified redaction fixes and
documented Kubernetes RBAC hardening without changing rule semantics,
schemas, fingerprints, scores, gates, rollback recommendations, or exit
codes. v1.3.0 remains the feature release for context-aware upgrade gating,
findings schema `1.1` with legacy `1.0` normalization, and the
`not_re_evaluated` comparison bucket; the real-EKS validation constant
should still move only after fresh real-EKS validation or recertification,
not merely because a new release shipped.
