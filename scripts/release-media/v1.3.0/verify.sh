...
if grep -rEqn "${PATTERN}" assets record-browser.mjs render.sh 2>/dev/null; then
  echo "FAIL: sensitive-identifier pattern found in demo source"
  grep -rEn "${PATTERN}" assets record-browser.mjs render.sh 2>/dev/null
  fail=1
else
...
echo
echo "== MP4: format/duration/faststart =="
...
check "poster is 1080x1080 (got ${w}x${h})" "$([ "${w}" = "1080" ] && [ "${h}" = "1080" ] && echo 1 || echo 0)"
